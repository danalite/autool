...
$folderPath = "$env:USERPROFILE\.node-gyp\16.20.0"
...
Invoke-WebRequest -Uri "https://nodejs.org/download/release/v16.20.0/win-x64/node.lib" -OutFile "$folderPath\Release\node.lib"

# Download node headers.tar.gz
Invoke-WebRequest -Uri "https://nodejs.org/download/release/v16.20.0/node-v16.20.0-headers.tar.gz" -OutFile "$folderPath\node-v16.20.0-headers.tar.gz"

# Extract the .gypi files into the base folder
tar -xf "$folderPath\node-v16.20.0-headers.tar.gz" -C $folderPath

# Copy $folderPath\node-v16.20.0\include\node\*gypi to $folderPath
